...
function TotalCarmap({data, topo}) {
    ...
    function handleDownload() {
        ...
        saveSvgAsPng(svgElement, `korea-year-carmap-${year}.png`, {
        ...
        const provinces = Array.from(grouped, ([codePrefix, feats]) => {
            ...
            return {
                ...
                const [vmin, vmax] = values.length ? d3.extent(values) : [0, 30];
                    // 기온 높을수록 빨강, 낮을수록 파랑 (reverse)
                const color = d3
                    .scaleSequential()
                    .domain([vmin, vmax]) // 낮은값→밝은파랑, 높은값→진한파랑
