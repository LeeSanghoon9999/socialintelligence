...
function SeasonalPM10map({data, topo}) {
    ...
    function handleDownload() {
        ...
        saveSvgAsPng(svgElement, `korea-season-pm10map-${season}.png`, {
    ...
    useEffect(() => {
        ...
        // 컬러 스케일 생성 (값 개수에 따라 처리)
        let color = d3.scaleQuantize()
            .domain(d3.extent(values))
            .range(d3.schemeOranges[7]);
        // 값이 1개면 quantize가 먹히지 않으니, 단일값 처리
        if (values.length <= 1) {
            color = () => d3.schemeOranges[7][3];
        }
        ...
        const tooltip = d3.select("body").append("div")
            .attr("class", "seasonal-map-tooltip")
        ...
        g.selectAll("path")
            ...
            .on("mouseover", function (e, d) {
                ...
                tooltip
                    .html(`<strong>${d.properties.province}</strong><br/>PM10: ${v != null ? v.toFixed(1) + " µg/m³" : "N/A"}`)
                    .style("opacity", 1);
            })
            .on("mousemove", (e) => {
                tooltip
    ...
    }, [season, data, topo]); // <-- 여기에 data, topo 반드시 추가!
...
export default SeasonalPM10map;